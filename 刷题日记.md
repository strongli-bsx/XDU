...
```cpp
  class Solution {
  public:
	  vector<string> combinationSum3(string digits) {
		  s.clear();
		  result.clear();
...
```cpp
  ...
  private:
	  void backTracking(string& s, int start, int end) {
		  if (start >= end) {
			  res.push_back(temp);
			  return;
		  }
		  for (int i = start; i < s.size(); i++) {
			  //每次循环都判断一下自身是否是回文串，然后在进行分割
			  if (isPalindrome(s, start, i)) {
				  string str = s.substr(start, i - start + 1); //(起始位置，长度）
				  temp.push_back(str);
			  } else continue;
			  backTracking(s, i + 1, s.size());
			  temp.pop_back();
		  }
	  }
	  //判断是否是回文串
