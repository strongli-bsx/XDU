...
**思路：**倒序比较，先满足最大孩子的需要，在分发下面的，这样就保证了每次都能有最优的解。

```cpp
//本题不用写双循环，饼干是在满足条件的时候才做自减运算，不是一直循环递减
class Solution {
public:
...
public:
	int findContentChildren(vector<int>& g, vector<int>& s) {
		...
		sort(s.begin(), s.end());
		int count = 0;
		int index = g.size() - 1;  //孩子食量的下摆哦
		for (int i = s.size() - 1; i >= 0; i--) {
            //遍历找到与饼干相匹配的人的下标
			while (index >= 0 && g[index] > s[i])  index--;
            //匹配上了，计数 + 1， 同时都自减寻找下一个
			if (index >= 0 && g[index] <= s[i]) {
				count++;
				index--;
			}
...
**思路：**检查元素的左右两侧，保证他们不同时大于或小于该元素即可。
...
**思路：**当前连续和为负数的时候立刻放弃，从下一个元素重新计算连续和，因为负数加上下一个元素 连续和只会越来越小。
...
**上面是用动态规划来做的，现在用贪心算法来做。**

```cpp
//每天可以买入卖出同时进行，故只需要计算正值即可
class Solution {
public:
	int maxProfit(vector<int>& prices) {
		int result = 0;
...
```cpp
//因为一定是走到终点的，故每移动一次，都要更新最大距离，直到nums.size() - 1，只需在走1步
class Solution {
public:
	int jump(vector<int>& nums) {
		...
		for (int i = 0; i < nums.size() - 1; i++) {
			...
			nextReach = max(i + nums[i], nextReach);
             //当上一次的最大距离走完的时候，才走保存的最大距离（当然这里是重合的）
             //显然此时才是更新步数的时候  相当于继续走最大值与上一次终点处的差值
			if (i == reach) {
				step++;
				reach = nextReach;
			}
...
**题目：**给定一个整数数组 A，我们只能用以下方法修改该数组：我们选择某个索引 i 并将 A[i] 替换为 -A[i]，然后总共重复这个过程 K 次。（我们可以多次选择同一个索引 i。）

以这种方式修改数组后，返回数组可能的最大和。

**思路：**贪心的思路，局部最优：让绝对值大的负数变为正数，当前数值达到最大。
...
**思路：**如果总消耗的油量等于提供的油量，则必可以跑完全程；其中如果某点提供的油量必须大于消耗的。
...
public:
	int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
		...
		for (int i = 0; i < gas.size(); i++) {
			sumGas += gas[i] - cost[i];  //从该位置求当天剩余油量
			totalGas += gas[i] - cost[i]; //总油量
			if (sumGas < 0) {
				start = i + 1;  //记录起始位置
				sumGas = 0;
			}
...
**思路：**这道题目一定是要确定一边之后，再确定另一边。两次遍历给糖果。

```cpp
//根据[1,3,2,2,1]跟[1,3,4,5,2]来分析代码
class Solution {
public:
	int candy(vector<int>& ratings) {
		...
		for (int j = ratings.size() - 2; j >= 0; j--) {
			if (ratings[j] > ratings[j + 1]){
                //如果当前节点的值本来就大于后节点的值，则不需要自增，只需要保持即可
                candyVec[j] = max(candyVec[j + 1] + 1, candyVec[j]);
            }
		}
		int result = accumulate(candyVec.begin(), candyVec.end(), 0);
		return result;
	}
};
```
...
**思路：**本题是大于等于自己身高的人数，所以采用降序，然后向里面插入数值。

```cpp
class Solution {
public:
    // 身高从大到小排（身高相同k小的站前面）
    //实现原理，送入数值[7,0] [7,1]来排序，首先比较第一个数，若相同，比较第二个数
...
public:
	...
	int findMinArrowShots(vector<vector<int>>& points) {
		...
		for (int i = 1; i < points.size(); i++) {
			//若当前的左端  > 前面的右端，则需要多一支箭
			if (points[i][0] > points[i - 1][1])  arrow++;
			//若两者之间有重合，取最小的值，查看是否还有重合
             //若没有，则跟上一个相同
...
**思路：**反向考虑，先求出不重叠的区间，这个比较简单，然后总的区间数减去不重叠的区间数。
...
public:
	...
	int eraseOverlapIntervals(vector<vector<int>>& intervals) {
		...
		int  count = 1;   //非重叠区间计数
		int end = intervals[0][1]; //当前区间的右边界
		for (int i = 1; i < intervals.size(); i++) {
			if (end <= intervals[i][0]) { //下一左边界 > 当前左边界
				end = intervals[i][1];
				count++;
			}
		}
...
**思路：**求出每个字母最后出现的位置，由此做为每个字母的分割界限。
...
**思路：**按照左边界排序，排序之后局部最优：每次合并都取最大的右边界。如第一个区间的右边界大于第二个左边界，则合并成右边界（选最大的值）。
...
public:
	...
	vector<vector<int>> merge(vector<vector<int>>& intervals) {
		...
		int length = intervals.size();
         //从i = 1开始，start为 i = 0 时的区间起始位置
		for (int i = 1; i < length; i++) {
			int start = intervals[i - 1][0];
			int end = intervals[i - 1][1];
			while ( i < length && intervals[i][0] <= end) {
                 //取最大的右端值，因为有可能给的区间正好是上一个空间的子空间
        ...
        //如果最后一个区间没有被合并，则添加到结果集中
		if (flag == false)  result.push_back({ intervals[length - 1][0],
...
**思路：**如果原数字最高位大于次高位（递减），则此数必须最高级降一级，为X 9 ···。
...
public:
    int monotoneIncreasingDigits(int N) {
		...
		int flag = strNum.size();
         //从后向前遍历来更改，从前向后遍历也可以，但是更改后还要再检查是否更改的正确
		...
		for (int j = flag + 1; j < strNum.size(); j++) {
			strNum[j] = '9';  //要从递减的下一位开始赋 9
		}
		return stoi(strNum);
	}
};
...
public:
		...
		for (int i = 1; i < prices.size(); i++) {
			if (prices[i] < minPrice)  minPrice = prices[i];  //找最小买入点
			//如果发现赔钱，则不卖
			if (prices[i] >= minPrice && prices[i] <= minPrice + fee) continue;
			if (prices[i] > minPrice + fee) {
				profit += prices[i] - minPrice - fee;  //卖了
				minPrice = prices[i] - fee;  //相当于以当前利润可以买一支股票
			}
		}
		return profit;
	}
...
- 整体复习一下之前所做过过得题目，整理一下思路，归纳总结
- 看一遍剑指offer，做一下笔记，学习一下思路；顺便看一下底层，即OS
- 补充C++基础知识，比如继承类的知识点，上次听别人面试好多都不会
...
> 滑动窗口在循环找窗口时，要记得减去left 的值，实质上只有右边界在移动。
...
```cpp
//忘记当时从哪里看的了，思路很清晰：一开始令index = 0，若初始值比target大，则返回处置位置
//若在数组中间，则找到返回比target大的第一个数的下标
//若在最后，则返回数组的长度
class Solution {
public:
	int searchInsert(vector<int>& nums, int target) {
...
public:
	int searchInsert(vector<int>& nums, int target) {
        ...
        //left一定会走到比right大的值的位置，此时才会退出循环
		return left;
	}
};
```

> 二分法退出循环的条件：一定是left ＞ right才退出。
...
==二分法时间复杂度还是减少一半，好好理解一下。==

```cpp
//先写一个自己写的算法
//本质上使用标记，如果找到合适的元素，就标记一下，直到下一个不相同的的元素
//这里有2种额外的情况要处理，一种是当元素为数组末尾的时候，那么结果结束的下标就是数组末尾
//另一种是当只有一个元素，且位于初始位置时候，第二个if会一直进去修改结果值
class Solution {
public:
	vector<int> searchRange(vector<int>& nums, int target) {
...
public:
	vector<int> searchRange(vector<int>& nums, int target) {
        ...
        //没找到相同的值，就返回；其中注意，left = mid + 1，而right = left = 末尾
        //此时left 会溢出
...
public:
	vector<int> searchRange(vector<int>& nums, int target) {
		...
		if (left == nums.size()) return result;
		while (right >= 0) {
...
```cpp
//一开始觉得不用二分法，直接从前向后算，时间复杂度也不高，其实错了。
//当数据相当庞大的时候，二分法优势就出来了
class Solution {
public:
	int mySqrt(int x) {
		int left = 0, right = x;
...
```cpp
//这里要注意，本题是升序数组，故不存在重复元素，可以不考虑等号
//升序数组，意味着右侧的数要大于左侧的数
//旋转数组中，如果left选mid右边，right选mid左边，这样会漏值
class Solution {
public:
...
public:
	vector<int> spiralOrder(vector<vector<int>>& matrix) {
		...
		while (true) {
			...
			if (++up > down)  break; //这里up做了自加运算，等价于从下一行开始
...
> 两个要注意的点： 
>
> 1. 要设置虚头结点，虚节点的初始化为 `new ListNode(0)`，不能写成NULL，那样是空节点，即无效；
> 2. 定义了虚节点后，要用另一个变量来遍历链表，即用`curr`，这样在返回的时候，才能返回链表
>    否则，返回`curr->next`的时候会是空节点，因为`curr`遍历到最后的位置了。
...
> 注意：
>
> 1. 链表的index是从0开始计数的，0对应着头节点；
>
> 2. 加了虚头结点后的原链表的长度为 size ，index = size - 1。
...
public:
    ...
    void addAtIndex(int index, int val) {
        if(index > size)  return;
        ListNode* curr = dummyHead;
        while(index--){ //因为新链表加了一个节点，故index的位置是原位置的前一位，正好
            curr = curr->next;
        }
        ListNode* node = new ListNode(val);
        node->next = curr->next;
...
public:
	ListNode* reverseList(ListNode* head) {
		ListNode* pre = NULL;
		ListNode* curr = head;
        //这里虽然会有temp = NULL，即最后节点指向的是空节点，
        //但是返回的值是pre，即空节点前面的值做为头节点
		while (curr) {
			ListNode* temp = curr->next;
			curr->next = pre;
			pre = curr;
...
public:
	int fourSumCount(vector<int>& nums1, vector<int>& nums2, vector<int>& nums3, 
		...
		for (auto e : nums3) {
			for (auto c : nums4) {
				if (map1.find(-e - c) != map1.end()) {
					//count++;  // 这里不能这样写成++，因为是计算组合数，会漏解
					count += map1[0 - (e + c)];
				}
			}
		}
...
> 本题双指针在找到满足的条件后，要记得做递增或者递减运算，这样才能结束循环。
>
> 别忘了结果要去重。去重逻辑放在找到满足的结果后，判断下前后的值是否相同，若相同，则跳过。
        ...
        for (int i = 0; i < nums.size(); i++) {	
            ...
            while (left < right) {			
                if (nums[left] + nums[right] < target) {		
                    ...
                    while (left < right && nums[left] == nums[left + 1])  left++;					while (left < right && nums[right] == nums[right - 1])  right--;					left++;			
                    right--;			
                }		
            }		
        }		
    ...
    vector<vector<int>> fourSum(vector<int>& nums, int target) {	
        ...
        for (int i = 0; i < nums.size(); i++) {	
            ...
            for (int j = i + 1; j < nums.size(); j++) {	
                ...
                int right = nums.size() - 1;	
                while (left < right) {	
                    if (nums[left] + nums[right] > tar2)  right--;	
                    if (nums[left] + nums[right] < tar2) left++;	
                    if (nums[left] + nums[right] == tar2) {		
...
> 该题目分为两步： 第一步是去除多余的空格，第二步则是遇到空格就反转。
...
> 从效果上看，字符串重排后定义大小，耗费的时间资源远小于直接删除空白字符。
...
public:	
    int strStr(string haystack, string needle) {
        ...
        for (int i = 0; i < haystack.size(); i++) { // 注意i就从0开始	
            ...
            if (haystack[i] == needle[j]) { // 因为是下标0开始，所以不用后移1位 	
                j++;		
            }		
            if (j - 1 == (needle.size() - 1)) { // 文本串s里出现了模式串t	
                return (i - needle.size() + 1);		
            }		
        }		
        return -1;	
    ...
    void getNext(vector<int>& next, const string needle) {		
        ...
        for (int i = 1; i < needle.size(); i++) {    
            while (j >= 1 && needle[i] != needle[j]) {	
                j = next[j - 1];  //j 不能从0 开始的原因：aabaaa，0101220		
            }		
...
> 双指针目前做到解题方法有两种：一种是快慢指针，一种是首尾指针。
...
string replaceSpace(string& s){ 
    ...
    s.resize(s.size() + space * 2); 
    int newSize = s.size(); 
    for(int left = oldSize - 1, right = newSize - 1; left >= 0; left--, right--){  
        if(s[left] != ' '){  
            s[right] = s[left];   
        }else{         
            s[right--] = '0';    
...
private:
    void deleteSpace(string& s) {	
        if (s.size() == 0)  return;	
        int fast = 0, slow = 0;	
        while (fast < s.size() && s[fast] == ' ') fast++; //删除首空格	
        for (; fast < s.size(); fast++) {	
...
public:  
    ListNode *detectCycle(ListNode *head) {   
        ListNode* fast = head;       
        ListNode* slow = head;    
        //之所以判断fast不为NULL，是为了保证 fast->next->next是合法的    
        //例如：1->2，fast = 2， fast->next = NULL，fast->next->next是非法的    
        while(fast != NULL && fast->next != NULL) {       
            fast = fast->next->next;          
            slow = slow->next;        
            if(slow == fast){      
...
> 本题中的首尾指针，满足的条件是==互斥==的，不能用并列条件，因为执行语句里面涉及了变量的变化。
            ...
            for (int j = i + 1; j < nums.size(); j++) {		
                ...
                while (left < right) {		
                    if (nums[left] + nums[right] == tar2) {		
                        ...
                        left++;		
                        right--;	
                    } else if (nums[left] + nums[right] > tar2)   right--;	
                    else  left++;	
                }		
            }	
        }	
...
> 如果先入栈： 1，2，3，4，此时出栈 4；在入栈5，则出栈5；这不符合队列。
>
> 队列要求是先入先出，故入栈的要全部出栈后才能继续入栈，继续出栈，保证了先入先出。
...
public: 
    ...
    //要注意，首先判断stk2的元素要全部出栈，才能执行下次的入栈   
    int pop() {   
        if(stk2.empty()){ 
            while(!stk1.empty()){    
                stk2.push(stk1.top());       
...
> 排除几个失败项：首先确定要判断的在不在map内；其次判断几个false；最后返回判断。
...
```cpp
//string 正好提供了入栈跟出栈的接口，跟上面相比，就是少了将最后的结果拼接与反转
class Solution {
public:
    string removeDuplicates(string S) {	 
        string str;      
...
> 一个要注意的点，输入的是字符串类型的数字，但是我们要计算的是数字int类型，所以要转换。
...
>  **优先队列：**优先队列时一种比较重要的数据结构，它是有二项队列编写而成的，可以以O(log n) 的效率查找一个队列中的最大值或者最小值，其中是最大值还是最小值是根据创建的优先队列的性质来决定的。
>
>  在STL中，默认情况下（不加后面两个参数）是以vector为容器，以 operator< 为比较方式，所以在只使用第一个参数时，优先队列默认是一个最大堆，每次输出的堆顶元素是此时堆中的最大元素。
>
>  ```cpp
>  //构造一个空的优先队列（此优先队列默认为大顶堆）priority_queue<int> big_heap;   //另一种构建大顶堆的方法priority_queue<int,vector<int>,less<int> > big_heap2; //构造一个空的优先队列,此优先队列是一个小顶堆priority_queue<int,vector<int>,greater<int> > small_heap;  
>  ```
>
>  
...
public:	
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        ...
        for (int i = k; i < nums.size(); i++) {	
            q.emplace(nums[i], i);   
            //while一般是用不到的，来了比之前大的元素就放到队首，即top处，故一般不会pop  
            //之所以有这个是为了防止该窗口为1的时候，ta本省就是最大值，需要pop			
            while (q.top().second <= i - k)  
                q.pop();			
            result.push_back(q.top().first);	
        }		                                                                
...
```cpp
//迭代 1push，1pop，5push，2push，2pop，4push，3push，3pop，4pop，5pop，7push，6push
//6pop，7pop
class Solution {
public:
...
class Solution {
 public:	vector<int> postorderTraversal(TreeNode* root) {
        ...
        while (!stk.empty()) {	
            TreeNode* curr = stk.top();		
            stk.pop();		
            if (curr != NULL)  result.push_back(curr->val);	
            else continue;			
            stk.push(curr->left);	
            stk.push(curr->right);	
        }	
...
```cpp
//中序遍历·与之前的区别在于，之前需要遍历到左子树的最左节点，然后进行处理操作//当下是直接向stack中赋值，且每个节点后面都会添加NULL来标记。
class Solution {
 public:
    vector<int> preorderTraversal(TreeNode* root) {
     stack<TreeNode* > stk;	
    ...
    vector<vector<int>> levelOrder(TreeNode* root) {	
        ...
        while (!que.empty()) {	
            ...
            while (size--) {	
                TreeNode* curr = que.front();	
                que.pop();			
                if (curr != NULL) vec.push_back(curr->val);		
                else continue;			
                if (curr->left)  que.push(curr->left);		
                if (curr->right) que.push(curr->right);		
            }		
...
> return返回的值需要＋ 1时，要写成 return ++depth，
...
> ==很典型的栈的使用==：记录路径，不能使用其他的容器（string），因为每次循环的时候，都是从stack取出上一次的值，拼接好路径后又入栈，这样保留每次的路径。
...
public:
    vector<string> binaryTreePaths(TreeNode* root) {
        ...
        while (!stk.empty()) {	
            ...
            if (curr->right) {	
                stk.push(curr->right);	
                path.push(str + "->" + to_string(curr->right->val));	
            }			
            if (curr->left) {	
                stk.push(curr->left);	
                path.push(str + "->" + to_string(curr->left->val));	
            }		
        }	
        return result;	
    }
...
> 注意：这里有两个递归的地方，一个是向下找相同的子树，一个是对比树是否相同。
...
public:
    ...
    bool compare(TreeNode* p, TreeNode* q) {	
        ...
        bool leftTree = compare(p->left, q->left);	
        bool rightTree = compare(p->right, q->right);	
...
public:
    bool hasPathSum(TreeNode* root, int targetSum) {
        ...
        while (!stk.empty()) {	
            ...
            if (curr->left) {	
                stk.push(curr->left);	
                path.push(curr->left->val + sum);	
            }		
            if (curr->right) {		
                stk.push(curr->right);	
                path.push(curr->right->val + sum);
            }	
        }		
        return false;
    }
            ...
            if (!curr->left && !curr->right) {	
                ...
                for (int i = 0; i < str.size(); i++) {	
                    if (str[i] == '.') {	
                        ...
                        temp.push_back(str.substr(start, end));	
                    }				
                    if (i == str.size() -  1) {	
                        temp.push_back(str.substr(end + 1, str.size()));
                    }			
                }					
                for (int i = 0; i < temp.size(); i++) {		
                    vec.push_back(stoi(temp[i]));	
                }			
                if (accumulate(vec.begin(), vec.end(), 0) == targetSum) {	
                    result.push_back(vec);		
                }			
            }			
            if (curr->left) {	
...
> traversal每次都从两个遍历的数组中取出一个节点的值，依次递归。
...
public:
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {	
        if (inorder.size() == 0 || postorder.size() == 0)  return NULL;		
        return traversal(inorder, 0, inorder.size() - 1, postorder, 0, 
                         postorder.size() - 1);	
    }	
    
    TreeNode* traversal(vector<int>& inorder, int startInorder, int endInorder,	
                  vector<int>& postorder, int startPostorder, int endPostorder)  {	
        if (startPostorder > endPostorder)  return NULL;	
        //根节点		
        int rootValue = postorder[endPostorder];	
        TreeNode* root = new TreeNode(rootValue);	
        if (endPostorder - startPostorder == 0)	 return root;	
        //分割点		
        int j;		
        for (j = 0; j < inorder.size(); j++) {	
            if (inorder[j] == rootValue)  break;	
        }	
        //分割	
        int startLeftInorder = startInorder;
        int endLeftInorder = j  - 1;	
        int startRightInorder = j + 1;		
        int endRightInorder = endInorder;	
        //后序数组要删掉最后一个元素	
        int startLeftPostorder = startPostorder;	
        int endLeftPostorder = startPostorder + endLeftInorder - startInorder;	
        int startRightPostorder = endLeftPostorder + 1;		
        int endRightPostorder = endPostorder - 1;	
        root->left = traversal(inorder, startLeftInorder, endLeftInorder, 	
                               postorder, startLeftPostorder, endLeftPostorder);
        root->right = traversal(inorder, startRightInorder, endRightInorder,	
                               postorder, startRightPostorder, endRightPostorder);	
        return root;
    }
};
```
...
public:	
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {	
        if (preorder.size() == 0 || inorder.size() == 0)	
            return NULL;	
        return traversal(preorder, 0, preorder.size() - 1, 
    ...
    TreeNode* traversal(vector<int>& preorder, int preorderStart, int preorderEnd,		vector<int>& inorder, int inorderStart, int inorderEnd) {	
        ...
        TreeNode* root = new TreeNode(rootValue);	
        if (inorderEnd == inorderStart)  return root;	
        //分割点	
        int j;		
        for (j = 0; j < inorder.size(); j++) {		
            if (inorder[j] == rootValue) break;	
        ...
        int leftInorderEnd = j - 1;		
        int rightInorderStart = j + 1;		
        int rightInorderEnd = inorderEnd;		
        //前序数组第一个值要删掉		
...
```cpp
//两边都是闭区间，另外，right 是含在区间内的，所以可以取等号。
class Solution {
public:
    TreeNode* constructMaximumBinaryTree(vector<int>& nums) {
    ...
    TreeNode* traversal(vector<int>& nums, int left, int right) {	
        ...
        root->left = traversal(nums, left, j - 1);	
        root->right = traversal(nums, j + 1, right);		
        return root;	
    }
};
```
...
> 不能单纯的比较**左节点小于中间节点**，**右节点大于中间节点**就完事了；
>
> 要比较的是 左子树==所有节点==小于中间节点，右子树==所有节点==大于中间节点。
...
public:
    int getMinimumDifference(TreeNode* root) {	
        if (root == NULL)  return 0;	
        vector<int> result;		traversal(root, result);	
        int ans = INT_MAX;		
...
public:	
    ...
    vector<int> result;	vector<int> findMode(TreeNode* root) {	
        if (root == NULL)  return result;	
        traversal(root);	
        return result;
    }	
...
//迭代法主要是将遍历方式更换成迭代遍历的方式，这题目的重点不在这儿，所以不写了
```
...
> 本题使用后序遍历的方法，因为后序遍历是天然的回溯。看代码很抽象，自己模拟一下过程就清晰了。
...
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {	
        ...
        TreeNode* left = lowestCommonAncestor(root->left, p, q);	
        TreeNode* right = lowestCommonAncestor(root->right, p, q);   
        if (left && right)  return root;  //说明左右子树分别存在p，q	
        if (left && !right)  return left; //说明右子树中没有目标值	
        if (!left && right)  return right; //说明左子树中没有目标值	
...
```cpp
//递归
//只需要遍历树，然后将数值添加进去就好了，因为BST 是有序的
class Solution {
public:	
    TreeNode* insertIntoBST(TreeNode* root, int val) {	
        if (root == NULL) {		
...
public:	
    TreeNode* insertIntoBST(TreeNode* root, int val) {	
        ...
        TreeNode* curr = root; // 遍历用	
        TreeNode* pre = root; //记录节点用		
        while (curr) {		
            pre = curr;		
            if (curr->val > val)  curr = curr->left;	
            else  curr = curr->right;	
    ...
    TreeNode* deleteNode(TreeNode* root, int key) {		
        ...
        if (root->val == key) {		
            if (!root->left)  return root->right;	
                ...
                //将目标值的左子树放在右子树的左叶子节点下面		
                TreeNode* curr = root->right;  //找到目标值右子树中的左子树的叶子节点	
        ...
        //这个两步的操作是寻找要目标值       
        //不能写成TreeNode* left = deleteNode(), 这样是没有有递归的	
        if (root->val > key)  root->left = deleteNode(root->left, key);	
        if (root->val < key)  root->right = deleteNode(root->right, key);	
        return root;	
    }
...
public:	
    TreeNode* deleteNode(TreeNode* root, int key) {	
        if (root == NULL)  return NULL;		
        TreeNode* curr = root; 
        TreeNode* prev = NULL;  //记录目标的节点的父节点	
        while (curr) {		
    ...
    //将目标值左子树放到右子树的左叶子节点的下面
    TreeNode* delNode(TreeNode* curr) {	
...
> 回溯法本质上穷举牢牢把握住这点，先写约束条件，然后递归进行遍历所有可能。
>
> 回溯算法的经典题型：组合、排列。
>
> 回溯算法的核心在于怎么写遍历条件。
...
```cpp
//先放入1，然后开始组合剩下的可能， （1,2） （1,3） （ 1,4），然后将1删掉，放入2，以此…
class Solution {
public:
	vector<vector<int>> result;
	vector<int> vec;
...
**剪枝：**（回溯算法要考虑一下剪枝，优化一下问题的解）

```cpp
//每层循环的时候，若i 的起始位置 < 剩余的个数，则没必要进行循环了，故可以剪枝
//这里利用的是，已选取元素的个数 + 为选取元素的个数 = 总个数
//i表示本次的起始位置，并没有选择该元素
for(int i = start; i <= n - (k - 1 - vec.size()); i++)  //i 为本次搜索的起始位置
```
...
> 这个题目的剪枝要比上个题目容易些，因为计算总和的组合数的约束条件要严格些。
>
> 另外，对于 i 的剪枝，是向下一次循环传入参数时的最大值。
...
> 注意：编译器无法从==默认的成员初始化==表达式中推导数组界限，	即const string [10]，如果不写的话会报错。
...
private:
	void backTracking(const string& digits, int index) {
		...
		//所有可能
		//digits 是字符串格式，要转化成int
		int digInt = digits[index] - '0';
		string s = letter[digInt];  //将按键上的数字取出
		for (int i = 0; i < s.size(); i++) {
			str += s[i];
...
```cpp
//本质上就是穷举，重复的穷举一个数字，直到这个数字不满足要求为止，进入下一个数字的穷举
class Solution {
public:
	vector<vector<int>> result;
	vector<int> v;
	...
	void backTracking(string& s, int start) {
		...
		for (int i = start; i < s.size(); i++) {
			if (isPalindrome(s, start, i)) {
                ...
                //这里不能用start，因为第二次传入的时候，是3，用start的会循环传入2
				backTracking(s, i + 1);  
				v.pop_back();
			}
...
```cpp
//此题不是排列，故不用在函数外定义成员变量，因为每次都是从下一个位置开始的
//而排列每次都是从 0(下标) 处开始的
class Solution {
public:
...
private:
	...
	void backTracking(const vector<int>& nums, int start) {
		...
		for (int i = start; i < nums.size(); i++) {
            //flag[nums[i] + 100] : 用于去除本次递归已经选过的元素
            if((!v.empty() && nums[i] < v.back()) || flag[nums[i] + 100] == 1) {
                continue;
            }
			v.push_back(nums[i]);
			flag[nums[i] + 100] = 1;
...
> 排列是每次都是从头开始，所以不需要start，但需要一个memo。
>
> 排列的原理如下：1->2->3，1->3->2，2...，本质上每次都从 0（下标） 开始向v中添加，前一次添加过得元素都标记一下，然后递归给下层；当回溯的时候，将标记去掉，重新开始记录，如此反复。

**去重语句的两种写法：**

```cpp
if(flag[i] == 1)  continue; //说明递归下去的元素已经用过了，此时不再使用
if(flag[i] == 0){    //与上面的方法实现的功能一样，当递归的下去的元素没有使用过，则不用跳过
	函数语句;
}
```
...
private:
	void backTracking(const vector<int>& nums) {
		...
		for (int i = 0; i < nums.size(); i++) {
			...
			//flag = 1: 说明递归下去的时候用过了，所以跳过
			//nums[i - 1] = nums[i] : 说明递归的元素跟上次元素相同，可以跳过
            
            //下面这条语句是跳过首元素，从重复的元素（第二个）开始
            //如：1 1 2，从第二个 1 开始记录，每次从头开始，先添加 1(II) 
            //1(II)-> 1(I) -> 2，因为当添加中间元素的时候，怎么都不会执行这条语句
			if (i > 0 && flag[i - 1] == 1 && nums[i - 1] == nums[i]) continue;
			if (flag[i] == 0) {
				flag[i] = 1;
				v.push_back(nums[i]);
				backTracking(nums);
